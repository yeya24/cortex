...
func (cfg *PostingsCacheConfig) RegisterFlagsWithPrefix(prefix, block string, f *flag.FlagSet) {
	...
	f.DurationVar(&cfg.FetchTimeout, prefix+"expanded_postings_cache."+block+".fetch-timeout", 5*time.Minute, "Timeout for fetching postings from TSDB index when cache miss occurs. This prevents runaway queries from consuming resources when all callers have given up.")
...
func (c *blocksPostingsForMatchersCache) fetchPostings(blockID ulid.ULID, ix tsdb.IndexReader, ms ...*labels.Matcher) func(context.Context) (index.Postings, error) {
	...
	fetch := func() ([]storage.SeriesRef, int64, error) {
		...
		if cache.cfg.FetchTimeout > 0 {
			fetchCtx, cancel = context.WithTimeout(context.Background(), cache.cfg.FetchTimeout)
			defer cancel()
		}
