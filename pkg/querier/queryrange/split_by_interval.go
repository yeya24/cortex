...
type SplitByIntervalMiddlewareMetrics struct {
	splitByCount prometheus.Counter
}

func NewSplitByIntervalMiddleware(registerer prometheus.Registerer) *SplitByIntervalMiddlewareMetrics {
	return &SplitByIntervalMiddlewareMetrics{
		splitByCount: promauto.With(registerer).NewCounter(prometheus.CounterOpts{
			Namespace: "cortex",
...
func SplitByIntervalMiddleware(interval IntervalFn, limits Limits, merger Merger,
	metrics *SplitByIntervalMiddlewareMetrics) Middleware {
	if metrics == nil {
		metrics = NewSplitByIntervalMiddleware(nil)
	}
...
type splitByInterval struct {
	...
	// Metrics.
	metrics *SplitByIntervalMiddlewareMetrics
}
...
func (s splitByInterval) Do(ctx context.Context, r Request) (Response, error) {
	...
	s.metrics.splitByCount.Add(float64(len(reqs)))
